...
public class PodcastsBinder extends ViewBinder<Podcast> {
    ...
    public PodcastsBinder(Context context, BasePreferenceHelper prefHelper, DisplayImageOptions options, ToggleListner toggleListner, boolean isAutoDownload) {
    ...
    @Override
    public void bindView(Podcast entity, int position, int grpPosition, View view, Activity activity) {
        ...
        viewHolder.swtContinousPlay.setOnCheckedChangeListener((compoundButton, b) -> {
            if (viewHolder.swtContinousPlay.isPressed()) {
                toggleListner.onCheckedChanged(entity, position, b, AppConstants.Podcast);
            }
        });
